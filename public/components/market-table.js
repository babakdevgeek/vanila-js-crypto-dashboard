...
export default class MarketTable extends CustomTable {
  ...
  render(data) {
    ...
    const headings = [
      ["رتبه", "market_cap_rank"],
      ["دارایی", "name"],
      ["قیمت", "current_price"],
      ["تغییر ۲۴ساعته", "price_change_percentage_24h"],
      ["مارکت کپ", "market_cap"],
      ["حجم ۲۴ساعته", "total_volume"],
    ];
    ...
    headings.forEach(([label, key]) => {
      ...
      heading.append(
        create_element({
          tag: "button",
          class_names: ["sort-control"],
          attrs: { type: "button", "data-sort": key },
    ...
    if (Array.isArray(this.data)) {
      this.sortedCoins(this.data).forEach((coin) => {
        ...
        const coinCell = create_element({
          tag: "td",
          class_names: ["market-coin"],
          childrens: [
            create_element({
              tag: "img",
              attrs: { src: coin.image, alt: coin.name, loading: "lazy" },
            }),
            create_element({
              tag: "span",
              childrens: [
        ...
        const row = create_element({
          tag: "tr",
          attrs: { "data-id": coin.id },
          childrens: [
            create_element({
              tag: "td",
              text: format_number(coin.market_cap_rank, { to_persian: true }),
            }),
            coinCell,
            create_element({
              tag: "td",
              class_names: ["ltr"],
              text: `${currencyMark[this.currency]}${format_price(coin.current_price)}`,
            }),
            create_element({
              tag: "td",
              class_names: ["ltr", change >= 0 ? "success" : "danger"],
              text: `${format_number(change, { min_decimals: 2, max_decimals: 2 })}%`,
            }),
            create_element({
              tag: "td",
              class_names: ["ltr"],
              text: `${currencyMark[this.currency]}${format_number(coin.market_cap, { shorten: true })}`,
            }),
            create_element({
              tag: "td",
              class_names: ["ltr"],
              text: `${currencyMark[this.currency]}${format_number(coin.total_volume, { shorten: true })}`,
            }),
          ],
        });
    ...
    this.el_buttons_wrapper?.remove();
    this.el_buttons_wrapper = create_element({
      tag: "div",
      class_names: ["buttons-wrapper"],
    });
    this.loadPrevButton?.remove();
    if (this.page > 1) {
      this.loadPrevButton = create_element({
        tag: "button",
        text: "نمایش ۱۰ دارایی قبل",
        class_names: ["load-more"],
      });
      this.loadPrevButton.addEventListener("click", this.onLoadPrev);
      this.el_buttons_wrapper.append(this.loadPrevButton);
    }
    this.loadMoreButton?.remove();
    this.loadMoreButton = create_element({
      tag: "button",
      class_names: ["load-more"],
      text: "نمایش ۱۰ دارایی بعدی",
    });
    this.loadMoreButton.addEventListener("click", this.onLoadMore);
    this.el_buttons_wrapper.append(this.loadMoreButton);

    this.append(this.el_buttons_wrapper);
  }
}
